...
func (r *IngressReconciler) reconcileOneGatewayPerIngress(
	...
	for _, ingress := range ingresses {
		// Create Gateway with same name as Ingress
		gateway := r.synthesizeGatewayForIngress(&ingress)
		...
		// Create HTTPRoute
		httpRoute := r.synthesizeHTTPRoute(&ingress, gateway.Name)
...
func (r *IngressReconciler) reconcileSharedGateways(
	...
	for ingressClass, classIngresses := range ingressesByClass {
		...
		// Synthesize HTTPRoutes for each Ingress in this class
		for _, ingress := range classIngresses {
			httpRoute := r.synthesizeHTTPRoute(&ingress, gatewayName)
			if err := r.applyHTTPRouteIfManaged(ctx, httpRoute); err != nil {
				logger.Error(err, "failed to apply HTTPRoute")
				continue
			}
...
func (r *IngressReconciler) checkCertificateMatch(
	...
	// Check if the transformed hostname is in the TLS hosts
...
func (r *IngressReconciler) transformHostname(hostname string) string {
	// If no rewrite configured, return original
	if r.HostnameRewriteFrom == "" || r.HostnameRewriteTo == "" {
		return hostname
	}

	// Check if hostname ends with the "from" pattern
	if strings.HasSuffix(hostname, r.HostnameRewriteFrom) {
		...
		// If there's a prefix, add a dot separator
		if prefix != "" {
			return prefix + "." + r.HostnameRewriteTo
		}
		// If no prefix, just use the rewrite-to value
		return r.HostnameRewriteTo
	}

	// If hostname doesn't match the pattern, return as-is
	return hostname
}
...
func (r *IngressReconciler) filterAnnotations(annotations map[string]string, filters []string) map[string]string {
	if annotations == nil {
		return nil
	}
...
func (r *IngressReconciler) printGateway(gateway *gatewayv1.Gateway) {
	fmt.Println("=== Synthesized Gateway ===")
	fmt.Printf("Name: %s\n", gateway.Name)
	fmt.Printf("Namespace: %s\n", gateway.Namespace)
	fmt.Printf("Annotations: %v\n", gateway.Annotations)
	fmt.Printf("Listeners:\n")
	for _, listener := range gateway.Spec.Listeners {
		fmt.Printf("  - Name: %s, Hostname: %s, Port: %d, Protocol: %s\n",
			listener.Name, *listener.Hostname, listener.Port, listener.Protocol)
		if listener.TLS != nil {
...
func (r *IngressReconciler) applyHTTPRouteIfManaged(ctx context.Context, httpRoute *gatewayv1.HTTPRoute) error {
	...
	// Print HTTPRoute to stdout
	fmt.Println("=== Synthesized HTTPRoute ===")
	fmt.Printf("Name: %s\n", httpRoute.Name)
	fmt.Printf("Namespace: %s\n", httpRoute.Namespace)
	fmt.Printf("Annotations: %v\n", httpRoute.Annotations)
	fmt.Printf("ParentRefs:\n")
	for _, parentRef := range httpRoute.Spec.ParentRefs {
		fmt.Printf("  - Name: %s, Namespace: %s, SectionName: %s\n",
			parentRef.Name, *parentRef.Namespace, *parentRef.SectionName)
	}
...
func (r *IngressReconciler) synthesizeGatewayForIngress(ingress *networkingv1.Ingress) *gatewayv1.Gateway {
	gateway := &gatewayv1.Gateway{}
	gateway.Name = ingress.Name
	gateway.Namespace = r.GatewayNamespace
	gateway.Spec.GatewayClassName = "nginx"

	// Copy and filter annotations from Ingress
	gateway.Annotations = r.filterAnnotations(ingress.Annotations, r.GatewayAnnotationFilters)
	...
	// Collect hostnames with their TLS configurations from this Ingress only
	...
	for hostname, tlsConfig := range hostnameMap {
		...
		if tlsConfig != nil && tlsConfig.SecretName != "" {
			...
			if hostname != transformedHostname && !r.checkCertificateMatch(hostname, transformedHostname, tlsConfig.Hosts) {
...
func (r *IngressReconciler) synthesizeGateway(ingresses []networkingv1.Ingress, gatewayName string) *gatewayv1.Gateway {
	gateway := &gatewayv1.Gateway{}
...
func (r *IngressReconciler) synthesizeHTTPRoute(
	...
	// Copy and filter annotations from Ingress
	httpRoute.Annotations = r.filterAnnotations(ingress.Annotations, r.HTTPRouteAnnotationFilters)
	...
	for _, rule := range ingress.Spec.Rules {
		if rule.Host != "" {
			transformedHostname := r.transformHostname(rule.Host)
				...
				if path.Path != "" {
					...
					if path.PathType != nil {
						...
						case networkingv1.PathTypeImplementationSpecific:
							// Convert ImplementationSpecific to Prefix with warning
							logger.Info("Converting PathType ImplementationSpecific to PathPrefix for HTTPRoute compatibility",
